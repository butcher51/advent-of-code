const raw = `Game 1: 18 red, 8 green, 7 blue; 15 red, 4 blue, 1 green; 2 green, 17 red, 6 blue; 5 green, 1 blue, 11 red; 18 red, 1 green, 14 blue; 8 blue
...
Game 100: 2 blue, 8 green, 12 red; 2 green, 13 red; 2 red, 4 green; 2 green, 7 red; 10 green, 5 red, 1 blue`.split('\n');

// parse game data
const games = raw.map((row, i) => {
    const [gameId,rawSets] = row.split(':');
    
    const rawSetArr = rawSets.split(';');
    const sets = rawSetArr.map((rawSet, i) => {        
        return rawSet.split(',').map((cubes, i) => {
            const [count, color] = cubes.trim().split(' ');
            return {count: parseInt(count), color: color};
        });
        
    });
    return {
        gameId: parseInt(gameId.trim().split(' ')[1]),
        sets: sets
    };
...
function isGamePossible(game) {
    const possible = game.sets.every(set => {
        return set.every(cube => {
            return cube.count <= cubes[cube.color];
        });
    });
...
/** PART 2 */
function getColorMinimum(game, color) {
    return game.sets.map(set => {
        return set.find(cube => cube.color === color);
    }).sort((a, b) => b.count - a.count)[0].count;
}

function calculateMinimumCubeCount(game) {
    game.miniumns = {
        red: getColorMinimum(game, 'red'),
        blue: getColorMinimum(game, 'blue'),
        green: getColorMinimum(game, 'green'),
    };
    return game;
}


// console.log('games', JSON.stringify(calculateMinimumCubeCount(games[1]).miniumns, null, 2));

games.forEach(game => {
    calculateMinimumCubeCount(game);
});

let sum = 0;
games.map(game => {
   const power = game.miniumns.red * game.miniumns.blue * game.miniumns.green;
   sum += power;
})
